...
- [ ] **Implement Device Linking Endpoint: `POST /api/v1/patients/me/link-device`**
  - [ ] This endpoint will be protected and require a standard Firebase Auth ID Token.
  - [ ] Receive the device Serial Number (SN) and Device Number (DN) from the client.
  - [ ] Find the device in the external AirView database using the provided SN/DN.
  - [ ] If a matching device and patient profile are found, link the authenticated user's `firebase_uid` to that patient data in our Firestore.
  - [ ] Return a success or failure response to the client.
