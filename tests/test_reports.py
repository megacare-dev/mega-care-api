...
def setup_customer_find_mock(db_mock: MagicMock):
    """Helper to mock the customer lookup part."""
...
def test_get_report_by_date_success(client: TestClient, db_mock: MagicMock):
    ...
    """
    # Arrange
    db_mock = setup_customer_find_mock(db_mock)

    mock_report_doc = MagicMock()
    mock_report_doc.exists = True
    mock_report_doc.to_dict.return_value = {"rawData": MOCK_RAW_DATA}
    
    db_mock.collection.return_value.document.return_value.collection.return_value.document.return_value.get.return_value = mock_report_doc
...
def test_get_latest_report_success(client: TestClient, db_mock: MagicMock):
    ...
    mock_latest_report_doc.to_dict.return_value = {"rawData": MOCK_RAW_DATA}

    mock_query = MagicMock()
    mock_query.stream.return_value = [mock_latest_report_doc]
    
    db_mock.collection.return_value.document.return_value.collection.return_value.order_by.return_value.limit.return_value = mock_query
